...
void Gui::WindowLigthColor(glm::vec3& position, glm::vec3& color) const {
...
void Gui::WindowAddObject(TypeObject& type, glm::vec3& position, glm::vec3& size, glm::vec3& color, bool& state_button) const {
        ...
        if (ImGui::BeginListBox("Type object:", listbox_size)) {
            ...
            if (item_selected_idx > 0) {
                ...
                if (item_selected_idx == 1) {
                    type = TypeObject::kCube;
                }
                else if (item_selected_idx == 2) {
                    type = TypeObject::kPyramid;
                }
                else if (item_selected_idx == 3) {
                    type = TypeObject::kLight;
                }
            }
...
int Gui::WindowListObjects(const std::vector<SceneObject>& objects) const {
    static int item_selected_idx = 0;
...
void Gui::WindowLogs(Logs logs) const {
    ImGui::Begin("Logs");
