#include "AvionEngineEditor/widget/widget.hpp"
...
namespace avion::ui::widget {
  ...
  void Widget::Init() {
      ...
      io_.IniFilename = m_resman.GetResource<std::filesystem::path>("imgui.ini")->c_str(); 
  ...
  std::optional<std::string> Widget::w_AddModel() const 
  {
    static size_t item_selected_idx = 0;
    const auto& model_loaded_list = m_resman.GetModelLoadedList();
    
    std::string result;
    Begin("Model");
      ImVec2 listbox_size(350.0f, ImGui::GetTextLineHeightWithSpacing() * 4);
      if (ImGui::BeginListBox("##empty", listbox_size)) 
      {
        for (size_t i = 0; i < model_loaded_list.size(); ++i) 
        {
          const bool is_selected = (item_selected_idx == i);

          if (ImGui::Selectable(model_loaded_list[i].c_str(), is_selected)) 
          {
            item_selected_idx = i;
          }
          ...
          if (is_selected) {
            ImGui::SetItemDefaultFocus();
            if (item_selected_idx < model_loaded_list.size()) 
            {
              result = model_loaded_list[item_selected_idx];
            }
          }
        } 
      }
